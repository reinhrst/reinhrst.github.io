...
In the beginning, any app would do, including the built in Workouts app.
...
So about I year ago, I dropped the (bulky, never quite comfortable) iPhone from all my running workouts, and only use the watch.
...
However we see more apps these days that are "free", however they intend to be monetised.
They generally come with privacy statements that nobody ever reads (meaning: you probably agree for them to use your data), or there are In-App purchases needed to unlock functionality.

I *like* apps that ask for a (reasonably small) payment.
I understand them, I understand where they make their money, and I feel we join in a mutual agreement; I pay, they make sure the app keeps working (for a reasonable amount of time).
...
After a run around a lake in Germany, where I ended up climbing over fences, going through fields and jumping over ditches, because I lost the way, and Apple Maps only work when you have internet connection), I added one extremely important feature:
...
One of the (many) things that jump out for me though is the ability to import GPX tracks as routes (and getting alerts if you go off route).
...
The app has a 4.8 star rating in the app store (at time of writing), and costs less than a large coffee.
...
He replied to me within hours with detailed instructions on how to fix things (most of the problems were my own fault for not carefully reading the instructions), and promised to pick up the couple of true minor bugs that I found in the next release.
...
For the use that I'm getting from it, €5.99 is way too little for this app; if the author would have any "donate" button anywhere on the site, I would be more than happy to buy him a beer or two!
